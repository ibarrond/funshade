...
const uint8_t iv_aes_tiny[AES_BLOCKLEN] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
...
void G_sa(const uint8_t buffer_in[], uint8_t buffer_out[],
    ...
    MP_owf_aes128_standalone(iv_aes_tiny,buffer_in,buffer_out);
    // Process remaining blocks, using previous block as key
    for (size_t i = AES_BLOCKLEN; i < buffer_out_size; i+=AES_BLOCKLEN){
        MP_owf_aes128_standalone(&buffer_out[i-AES_BLOCKLEN],buffer_in,&buffer_out[i]);
    }
}