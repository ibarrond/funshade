...
static void aes128_enc_ecb(const struct AES_ctx* ctx, uint8_t* buf)
{
...
void MP_owf_aes128_standalone(const uint8_t key_in[], const uint8_t msg_in[], uint8_t msg_out[]){
    struct AES_ctx ctx;
    // Copy msg_in to msg_out with memcpy
    memcpy(msg_out, msg_in, AES_BLOCKLEN);

    AES_init_ctx(&ctx, key_in);
    aes128_enc_ecb(&ctx, msg_out);
...
void G_sa(const uint8_t buffer_in[], uint8_t buffer_out[],
    ...
    MP_owf_aes128_standalone(iv,buffer_in,buffer_out);
    // Process remaining blocks, using previous block as key
    for (size_t i = AES_BLOCKLEN; i < buffer_out_size; i+=AES_BLOCKLEN){
        MP_owf_aes128_standalone(&buffer_out[i-AES_BLOCKLEN],&buffer_in[i],&buffer_out[i]);
    }
}